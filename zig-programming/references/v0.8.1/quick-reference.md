...
| `nosuspend` | The `nosuspend` keyword can be used in front of a block, statement or expression, to mark a scope where no suspension points are reached. In particular, inside a `nosuspend` scope:  - Using the `suspend` keyword results in a compile error. - Using `await` on a function frame which hasn't completed yet results in safety-checked [Undefined Behavior](39-undefined-behavior.md#Undefined-Behavior). - Calling an async function may result in safety-checked [Undefined Behavior](39-undefined-behavior.md#Undefined-Behavior), because it's equivalent to `await async some_async_fn()`, which contains an `await`.  Code inside a `nosuspend` scope does not cause the enclosing function to become an [async function](35-async-functions.md#Async-Functions).  - See also [Async Functions](35-async-functions.md#Async-Functions) |
