...
[constant identifier](#Assignment), named `std`, that gives access the features of the Zig Standard Library.
...
(known as an [anonymous struct literal](#Anonymous-Struct-Literals)). The `\n`
...
Container level variables may be declared inside a [struct](15-struct.md#struct), [union](17-union.md#union), or [enum](16-enum.md#enum):
...
| `a ++ b` | - [Arrays](11-arrays.md#Arrays) | Array concatenation.  - Only available when `a` and `b` are [compile-time known](34-comptime.md#comptime). | `const mem = @import("std").mem; const array1 = [_]u32{1,2}; const array2 = [_]u32{3,4}; const together = array1 ++ array2; mem.eql(u32, &together, &[_]u32{1,2,3,4})` |
| `a ** b` | - [Arrays](11-arrays.md#Arrays) | Array multiplication.  - Only available when `a` and `b` are [compile-time known](34-comptime.md#comptime). | `const mem = @import("std").mem; const pattern = "ab" ** 3; mem.eql(u8, pattern, "ababab")` |
